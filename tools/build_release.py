...
def include(path: Path) -> bool:
    return not any(part in EXCLUDED or part.endswith(".pyc") for part in path.relative_to(ROOT).parts)
    ...
    with tarfile.open(archive, "w:gz", format=tarfile.PAX_FORMAT) as tar:
        for path in sorted(ROOT.rglob("*")):
            if path.is_file() and include(path):
                tar.add(path, arcname=Path(f"homehub-{VERSION}") / path.relative_to(ROOT), recursive=False)
